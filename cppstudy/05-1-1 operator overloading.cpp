/* 
기본 연산자들을 직접 사용자가 정의하는 것을 연산자 오버로딩이라고 한다.
같은 이름의 함수를 인자만 다르게 사용하는 것을 '함수 오버로딩'이라고 한 것처럼,
기본 연산자를 우리가 설계한 클래스에 맞게 직접 사용하는 것을 '연산자 오버로딩'이라고 한다.
...
int main() {
	...
	if (str2 == str3)
		...
		std::cout << "str2 와 str2 는 다르다." << std::endl;
}
